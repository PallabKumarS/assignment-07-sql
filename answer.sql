...
create table books(book_id SERIAL PRIMARY KEY,title VARCHAR(50) NOT NULL,author VARCHAR(50) NOT NULL,price DECIMAL NOT NULL CHECK(price > 0),stock SMALLINT NOT NULL,published_year VARCHAR(10) NOT NULL);

-- create customers table with columns and data types
CREATE TABLE customers(customer_id SERIAL PRIMARY KEY, name VARCHAR(50) NOT NULL, email VARCHAR(50) UNIQUE NOT NULL, joined_date DATE DEFAULT now());

-- create orders table with columns and data types
CREATE TABLE orders(order_id SERIAL PRIMARY KEY, customer_id INT NOT NULL REFERENCES customers(customer_id),book_id INT NOT NULL REFERENCES books(book_id),quantity SMALLINT NOT NULL CHECK (quantity > 0), order_date DATE DEFAULT now());
...
INSERT INTO customers (name,email) VALUES ('Pallab Kumar', 'pallabkumar26@gmail.com');
...
SELECT name,COUNT(order_id) as total_orders FROM orders NATURAL JOIN customers  GROUP BY name;

--* 4. total revenue generated by each book
SELECT SUM(quantity * price) as total_revenue FROM books NATURAL JOIN orders;

--* 5. customers who have ordered more than 1 order
SELECT name,COUNT(order_id) FROM customers NATURAL JOIN orders GROUP BY name HAVING COUNT(order_id) > 1;
